...
StorageTTL::StorageTTL()
  : _socket(NULL), _input(NULL), _name("Storage-TTL"), _argFormat(""), _description("Storage technique using the Time-To-Live field (transmit 1bit per packet)")
{
...
bool		StorageTTL::executeHiding()
{
  ...
  //Initialization of the algorithm Storage TTL
  this->_packet->getIPHeader()->set_ttl(63);
...
bool		StorageTTL::computeAndSendValue(uint8_t value)
{
  int		cpt = 0;
...
bool			StorageTTL::executeRecovering()
{
  int			sizeReceiv = 0;
  void			*data;
      ...
      if (this->_socket->socketReadable())
	{
	  ...
	  if (this->_packet->loadPacket(data, sizeReceiv))
	      ...
	      if (this->checkSource())
		{
		  ...
		  if (!initFinished)
		    {
		      if ((initFirstStep) && ((this->_packet->getIPHeader()->get_id() == (lastID + 1)) ||
					      (this->_packet->getIPHeader()->get_ttl() == (lastTTL + 1))))
			{
			  initFinished = true;
			  if (firstRound)
			    {
			...
			{
			  lastID = this->_packet->getIPHeader()->get_id();
			  lastTTL = this->_packet->getIPHeader()->get_ttl();
			  initFirstStep = true;
			}
		    }
		  //Algorithm
		    ...
		    {
		      valueTTL = this->_packet->getIPHeader()->get_ttl() - lastTTL;
		      oneByte = oneByte << 1;
		      if (valueTTL > 0)
			oneByte += 1;
		      ++cptByte;
...
void		StorageTTL::setRessources(ISocket *socket, IInput *input)
{
  this->_socket = socket;
  this->_input = input;
}
...
//Mutator method for the ressources of the algorithm
void		StorageTTL::setRessources(ISocket *socket, IOutput *output)
{
  this->_socket = socket;
  this->_output = output;
}
...
bool		StorageTTL::checkSyntax(std::vector<std::string> &args)
{
...
const std::string	&StorageTTL::getName() const
{
  return (this->_name);
}

//Accessor method for arguments format of the algorithm
const std::string	&StorageTTL::getArgFormat() const
{
  return (this->_argFormat);
}

//Accessor method for descritpion of the algorithm
const std::string	&StorageTTL::getDescription() const
{
  return (this->_description);
}

//Initialize all fields of headers within the packet
void			StorageTTL::initPacket()
{
